...
class Parser : IParser<Input>
{
    ...
    public Input Parse(string[] input)
    {
        ...
        [
            (
                Instruction: (Instruction)int.Parse(match.Groups[1].Value),
                ComboOperand: (ComboOperand)int.Parse(match.Groups[2].Value)
            ),
        ];
        for (var i = 0; i < match.Groups[3].Captures.Count; i++)
        {
            program.Add(
                (
                    Instruction: (Instruction)int.Parse(match.Groups[4].Captures[i].Value),
                    ComboOperand: (ComboOperand)int.Parse(match.Groups[5].Captures[i].Value)
                )
            );
        }
...
class Interpreter : ISolver<Input, string>
{
    ...
    public static IEnumerable<int> Execute(Input data)
    {
        ...
        long GetOperandValue(ComboOperand comboOperand) =>
            comboOperand switch
            {
                ComboOperand.Zero => 0,
                ComboOperand.One => 1,
...
class Solver : ISolver<Input, long>
{
    ...
    {
        wantedOutputs = data
            .Program.SelectMany(x => new[] { (int)x.Instruction, (int)x.ComboOperand })
            .ToArray();

        literal1 = (int)data.Program.First(x => x.Instruction == Instruction.Bxl).ComboOperand;
        literal2 = (int)
            data.Program.Where(x => x.Instruction == Instruction.Bxl).Skip(1).First().ComboOperand;
    ...
    List<long> FindA(List<(int Value, int Shift)> reserved, long candidate = 0, int output = 0)
    {
        ...
        for (var cShift = 0; cShift < 8; cShift++)
        {
            ...
            if (
                (newCandidate >> shift) % 8 != b
                || (newCandidate >> shift + cShift) % 8 != c
                || reserved.Any(x => (newCandidate >> x.Shift) % 8 != x.Value)
            )
            {
                continue;
            }
            ...
            solutions = solutions
                .Where(x => (x >> shift) % 8 == b && (x >> shift + cShift) % 8 == c)
                .ToList();
