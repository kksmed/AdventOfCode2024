...
class Parser : IParser<IEnumerable<ClawMachine>>
{
    ...
    public IEnumerable<ClawMachine> Parse(string[] input)
    {
        for (var i = 0; i < input.Length; i++)
        {
            ...
            yield return new(
                ParsePoint(line, regexA),
                ParsePoint(input[++i], regexB),
                ParsePoint(input[++i], regexPrize)
            );
        }
        yield break;
...
class Solver : ISolver<IEnumerable<ClawMachine>, int>
{
    ...
    IEnumerable<int> FindAllSolutions(ClawMachine claw)
    {
        for (
            var a = 0;
            a <= maxButtonPushes
                && a * claw.ButtonA.X <= claw.Prize.X
                && a * claw.ButtonA.Y <= claw.Prize.Y;
            a++
        )
        for (
            var b = 0;
            b <= maxButtonPushes
                && a * claw.ButtonA.X + b * claw.ButtonB.X <= claw.Prize.X
...
class Solver2 : ISolver<IEnumerable<ClawMachine>, long>
{
    ...
    long FindMinimum(ClawMachine claw)
    {
        ...
        if (
            maxAPushes * cm.ButtonA.X + maxBPushes * cm.ButtonB.X < cm.Prize.X
            || maxAPushes * cm.ButtonA.Y + maxBPushes * cm.ButtonB.Y < cm.Prize.Y
        )
        {
        ...
        if (
            gradeA < prizeGrade && gradeB < prizeGrade
            || gradeA > prizeGrade && gradeB > prizeGrade
        )
        {
        ...
        if (
            (cm.ButtonA.X + cm.ButtonA.Y) / (double)costOfA
            > (cm.ButtonB.X + cm.ButtonB.Y) / (double)costOfB
        )
        {
    ...
    {
        public CorrectedClawMachine(ClawMachine claw)
            : this(claw.ButtonA, claw.ButtonB, (claw.Prize.X + offSet, claw.Prize.Y + offSet)) { }
    }
}
