...
class Solver1 : ISolver<Data, int>
{
    ...
    protected static bool IsCorrectOrder(List<PageOrderingRule> rules, int[] pages)
    {
        for (var i = 1; i < pages.Length; i++)
        {
            ...
            if (
                rulesForPage
                    .Select(r => Array.FindIndex(pages, 0, i, p => p == r.After))
                    .Any(j => j > -1)
            )
            {
                return false;
...
class Solver2 : Solver1
{
    public override int Solve(Data data) =>
        data
            .Pages.Where(x => !IsCorrectOrder(data.Rules, x))
            .Select(x => FixOrder(data.Rules, x))
            .Select(Middle)
            .Sum();
...
class PageComparer(List<PageOrderingRule> rules) : IComparer<int>
{
    public int Compare(int x, int y)
    {
        var rule = rules.FirstOrDefault(r =>
            r.Before == x && r.After == y || r.Before == y && r.After == x
        );
        return rule is null ? 0
            : rule.Before == x ? -1
            : 1;
    }
}
