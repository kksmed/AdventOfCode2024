    ...
    public virtual int Solve(Data data)
    {
        ...
        while (unvisitedQueue.Count > 0)
        {
            ...
            foreach (var direction in Enum.GetValues<Direction>())
            {
                ...
                var newDistance =
                    Math.Min(current.ShortestPath + 1000, current.ShortestPaths[(int)direction])
                    + 1;
...
class Solver2 : Solver1
{
    public override int Solve(Data data)
    {
        ...
        foreach (
            var pd in current
                .ShortestPaths.Select((x, d) => (Path: x, Direction: (Direction)d))
                .Where(x => x.Path == current.ShortestPath)
        )
        ...
        void BackTrack(Node node, Direction direction)
        {
            ...
            var possibleDirections = new[] { direction, direction.Left(), direction.Right() }
                .Select(x => (Path: node.ShortestPaths[(int)x], Direction: x))
                .Select(x =>
                    x.Direction == direction || x.Path == int.MaxValue
                        ? x
                        : x with
                        {
                            Path = x.Path + 1000,
