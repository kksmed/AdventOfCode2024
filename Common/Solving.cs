...
{
  public static (TData? Example, TData? Input) Go<TData>(string? example, IParser<TData> parser, bool alsoParseInput = false) where TData : class
  {
  ...
  public static T1Result? Go<TData, T1Result>(string? example, IParser<TData> parser, ISolver<TData, T1Result> solver, bool solveInput = true) where TData : class
  {
    PrintDay(solver);
  ...
  public static (T1Result? Part1, T2Result? Part2) Go<TData, T1Result, T2Result>(
    ...
    ISolver<TData, T2Result>? solverPart2 = null) where TData : class
  {
  ...
  static T1Result? Solve<TData, T1Result>(ISolver<TData, T1Result> solver, TData? parsedExample, TData? parsedInput)
  {
