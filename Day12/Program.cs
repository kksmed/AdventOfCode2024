    ...
    )
    {
        ...
        if (goLeft.CanGo)
        {
            if (!used[goLeft.Point.X, goLeft.Point.Y])
            {
                ...
                var subResults = GetSidesAndArea(
                    map,
                    goLeft.Point,
                    plant,
                    used,
        ...
        if (goStraight.CanGo)
        {
            ...
            {
                var newEdge =
                    (!goLeft.CanGo ? Edge.LeftHandSide : 0)
                    | (!goRight.CanGo ? Edge.RightHandSide : default);
                var subResults = GetSidesAndArea(
                    map,
                    goStraight.Point,
                    plant,
            ...
            else
            {
                ...
                if (
                    ...
                    && !NextStep(
                        goStraight.Point,
                        TurnDirection(goStraight.Direction, Turn.Left)
                    ).CanGo
                )
                {
                    sides--;
                ...
                if (
                    ...
                    && !NextStep(
                        goStraight.Point,
                        TurnDirection(goStraight.Direction, Turn.Right)
                    ).CanGo
                )
                {
                    sides--;
        ...
        if (goRight.CanGo)
        {
            ...
            else
            {
                ...
                if (
                    !goStraight.CanGo
                    && !NextStep(goRight.Point, TurnDirection(goRight.Direction, Turn.Left)).CanGo
                )
                {
                    sides--;
                    if (verbose)
        ...
        (Point Point, Direction Direction, bool CanGo) NextStep(Point p, Direction d)
        {
            ...
            return (
                Point: nextPoint,
                Direction: d,
                CanGo: map.InBounds(nextPoint) && map[nextPoint.X, nextPoint.Y] == plant
            );
        }
    }
