...
public partial class Solver2 : ISolver<IEnumerable<IInstruction>>
{
  ...
  public IEnumerable<IInstruction> Parse(string[] input) =>
    input.SelectMany(x => regex.Matches(x))
      .Select(
        match => match.Groups[0].Value switch
          {
            "do()" => (IInstruction)new Do(),
            "don't()" => new Dont(),
            var mul when mul.StartsWith("mul", StringComparison.InvariantCulture) => new Mul(
