public static class AlternativeSolution
{
  ...
  private static long Part1(IReadOnlyList<long> input) => input.Select(n => GetSecretNumbers(n, 2000).Last()).Sum();
  ...
  private static IEnumerable<int> GetPrices(IEnumerable<long> secretNumbers) => secretNumbers.Select(n => (int)(n % 10));
  ...
  private static int BuyBananas(IReadOnlyList<int> prices, (int d1, int d2, int d3, int d4) trigger)
  {
    ...
    {
      if (
        priceChanges[i] == trigger.d1
        && priceChanges[i + 1] == trigger.d2
        && priceChanges[i + 2] == trigger.d3
        && priceChanges[i + 3] == trigger.d4
      )
      {
        return prices[i + 4];
      }
  ...
  private static long Part2(IReadOnlyList<long> input)
  {
    ...
    var pricesLists = input.Select(n => GetSecretNumbers(n, 1999)).Select(longs => GetPrices(longs).ToList()).ToList();
    var possibleTriggers = pricesLists
      .Select(prices => GetPriceChanges(prices).ToList())
