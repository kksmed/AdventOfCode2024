...
class Solver2 : ISolver<(Point Start, Element2[,] Warehouse, Direction[] Moves), int>
{
  ...
  static Element2[,] Move(Point robot, Element2[,] warehouse, bool print, params Direction[] moves)
  {
    if (print)
    {
  ...
  static Point? InnerMove(Point p, Direction d, Element2[,] w)
  {
    ...
    if (pToMove.X < 0 || pToMove.Y < 0 || pToMove.X >= w.GetLength(0) || pToMove.Y >= w.GetLength(1))
